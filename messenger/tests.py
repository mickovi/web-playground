...
# python manage.py test messenger (todos los tests)
# python manage.py test messenger.tests.ThreadTestCase (solo los tests de la clase ThreadTestCase)
# python manage.py test messenger.tests.ThreadTestCase.test_add_users_to_thread (solo el test de ese método)
...
class ThreadTestCase(TestCase):
    ...
    def test_add_message_from_user_not_in_thread(self):
        ...
        message3 = Message.objects.create(user=self.user3, content='xD')
        self.thread.messages.add(message1, message2, message3)
        self.assertEqual(len(self.thread.messages.all()), 2)

    # Test personalizado de object.manager
    def test_find_thread_with_custom_manager(self):
        self.thread.users.add(self.user1, self.user2)
        thread = Thread.objects.find(self.user1, self.user2)
        self.assertEqual(self.thread, thread)

    def test_find_or_create_thread_with_custem_manager(self):
        self.thread.users.add(self.user1, self.user2)
